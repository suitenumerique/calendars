...
const browserslist: string[] = (pkg as { browserslist: string[] }).browserslist;
...
export default defineConfig({
  plugins: [
    ...
    // (app + dependencies like `@event-calendar/core` that may use `toReversed` etc.),
    // so future ES20xx gaps are covered without hand-writing shims. `modernTargets`
    // is pinned to our browserslist so both the injected polyfills and esbuild's
    ...
    // Opt-in bundle analyzer: emits bundle-stats.html next to the project
    // root when ANALYZE=1. Run via `npm run analyze`.
