#!/usr/bin/env node
// Guardrail for the frozen dependency install (ported from
// suitenumerique/messages).
//
// Fails loudly if node_modules regresses, i.e. if:
...
if (problems.length) {
  ...
  console.error(
    "\nThe Adobe React Spectrum bloat has regressed. It is kept out by the\n" +
      'react-aria ecosystem pins in package.json "overrides". Verify those are\n' +
      "intact and run `npm run check:deps` after regenerating the lockfile.\n",
  );
  process.exit(1);
}

console.error(
  `\x1b[32m✓ node_modules OK\x1b[0m — ${gb.toFixed(2)} GB, no Adobe React Spectrum packages.`,
);
