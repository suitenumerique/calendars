...
import { buildPropfindBody, parseMultistatus, davRequest } from "../DavClient";
import { redirectToLogin } from "@/features/api/fetchApi";

const redirectToLoginMock = redirectToLogin as MockedFunction<typeof redirectToLogin>;
...
describe("buildPropfindBody", () => {
  ...
  describe("key validation", () => {
    it("throws on a key that breaks out of the element", () => {
      expect(() => buildPropfindBody({ "d:displayname/><evil x='": {} })).toThrow(
        /Invalid PROPFIND prop name/,
      );
    });

    it("throws on a key with whitespace", () => {
      expect(() => buildPropfindBody({ "d:bad name": {} })).toThrow(/Invalid PROPFIND prop name/);
    });

    it("throws on a key with a leading digit", () => {
      expect(() => buildPropfindBody({ "1bad": {} })).toThrow(/Invalid PROPFIND prop name/);
    });

    it("throws on a key with an attribute injection", () => {
      expect(() => buildPropfindBody({ 'd:x" attr="value': {} })).toThrow(
        /Invalid PROPFIND prop name/,
      );
    });

    it("throws on a key with multiple colons", () => {
      expect(() => buildPropfindBody({ "d:foo:bar": {} })).toThrow(/Invalid PROPFIND prop name/);
    });
...
describe("parseMultistatus", () => {
  ...
  it("surfaces per-resource <d:responsedescription> and <d:error>", () => {
    ...
    expect(responses[0].ok).toBe(false);
    expect(responses[0].responseDescription).toBe("Resource is locked by another user.");
