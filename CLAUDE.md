...
make lint-front         # Format (oxfmt) then lint (oxlint) the frontend
make format-front       # Format frontend sources with oxfmt only
make typecheck-front    # Run TypeScript type checker (tsc --noEmit)
make test-front         # Run frontend tests (Vitest)
cd src/frontend && npm test              # Run frontend tests (local)
cd src/frontend && npm run test:watch    # Watch mode (local)
```
...
A single Vite + React application (no monorepo/workspace):
- `src/features/` - Feature modules (calendar, auth, api, i18n, etc.)
- `src/routes/` - TanStack Router routes (route tree generated by `tsr generate`)
- `src/hooks/` - Custom React hooks

End-to-end tests live separately in `src/e2e/` (Playwright).
...
|---------|------------|-------------|
| **Frontend** | [http://localhost:8930](http://localhost:8930) | Vite Calendar frontend |
...
### Frontend
- Vite + React 19
- @tanstack/react-router for routing (file-based, route tree generated via `tsr generate`)
- @tanstack/react-query for data fetching
- @event-calendar/core for the calendar grid / scheduler views
- ts-ics for ICS parsing/generation; CalDAV HTTP handled by a custom `CalDavService` : https://github.com/Neuvernetzung/ts-ics
- @gouvfr-lasuite/cunningham-react + @gouvfr-lasuite/ui-kit for UI components : https://github.com/suitenumerique/cunningham
- oxlint (lint) and oxfmt (format) for code quality
